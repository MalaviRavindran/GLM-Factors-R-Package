#' gibbs_surv
#'
#' Function to perform factorized regression with the Exponential Survival Model
#' @import magrittr
#'
...
gibbs_surv <- function(y, X, nu, X_test = NULL, nrun = 1000, burn = 500, thin = 1,
  ...
  for(i in 1:nrun){
    ...
    for (h in 1:n){                # Metropolis hastings step
      ...
      logr = nu[h]*t(omega)%*%eta_star - nu[h]*t(omega)%*%eta[h,] -
        y[h]*exp(t(omega)%*%eta_star) + y[h]*exp(t(omega)%*%eta[h,]) +
    ...
    for (j in 1:k){                # Metropolis hastings step
      ...
      logr = sum(eta_omega_star*nu) - sum(eta_omega*nu) -
        sum(exp(eta_omega_star)*y) + sum(exp(eta_omega)*y) -
