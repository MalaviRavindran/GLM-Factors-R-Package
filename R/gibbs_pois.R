#' gibbs_pois
#'
#' Function to perform factorized Poisson regression
#'
#' @param y
...
#' @export
gibbs_pois <- function(y, X, X_test = NULL, nrun = 1000, burn = 500, thin = 1,
  ...
  for(i in 1:nrun){
    ...
    for (h in 1:n){                # Metropolis hastings step
      ...
      eta_star = bayesSurv::rMVNorm(1, eta[h,], diag(k)*delta_eta)

      logr =   stats::dpois(y[h], exp(t(omega)%*%eta_star), log = T) -
        stats::dpois(y[h], exp(t(omega)%*%eta[h,]), log = T) +
        mvtnorm::dmvnorm((X[h,] - Lambda%*%eta_star) %>% as.vector(), sigma = Sigma, log = T) -
        mvtnorm::dmvnorm((X[h,] - Lambda%*%eta[h,]) %>% as.vector(), sigma = Sigma, log = T) +
        mvtnorm::dmvnorm(eta_star, log = T) -
    ...
    for (j in 1:k){                # Metropolis hastings step
      ...
      omega_star <- omega
      omega_star[j] <- stats::rnorm(1, omega[j], delta_omega)

      eta_omega_star <- eta%*%omega_star

      logr =
        sum(stats::dpois(y, exp(eta_omega_star), log = T)) -
        sum(stats::dpois(y, exp(eta_omega), log = T)) +
      ...
      if (logr > logu){
        ...
        eta_omega <- eta_omega_star
        omega[j] = omega_star[j]
